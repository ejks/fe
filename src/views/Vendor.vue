<template>
	<div class="vendor">
				...
				<v-dialog v-model="dialog" max-width="290">
					<v-card>
						...
						<v-form ref="form" v-model="valid" lazy-validation class="mx-5">
							...
							<v-file-input label="Deal Photo" prepend-icon="mdi-camera">
							</v-file-input>
						</v-form>
		...
		<v-layout row class="mx-9">
			<v-list-item>
				<v-list-item-content>
					<v-list-item-title class="title font-weight-bold mb-1 mt-n4"
...
<script>
	...
	export default {
		...
		created() {
			axios
				.get('http://localhost:3000/api/v1/vendors.json')
				.then(response => {
